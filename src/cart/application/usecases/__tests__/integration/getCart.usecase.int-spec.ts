...
describe('GetCartUseCase integration tests', () => {
  const prismaService = new PrismaClient();
  let sut: GetCartUseCase.UseCase;
  ...
  beforeEach(async () => {
    sut = new GetCartUseCase.UseCase(repository);
  ...
  it('Should get a cart', async () => {
    ...
    const createCart = await prismaService.cart.create({
      data: cartEntity,
    });
    ...
    expect(output).toStrictEqual(createCart);
  });
});
